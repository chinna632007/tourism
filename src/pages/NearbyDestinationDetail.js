    ...
    <div style={{ minHeight: '100vh' }}>
      ...
      <div className="destination-detail-hero">
        ...
        <div style={{
          ...
          <p style={{ fontSize: '14px', marginBottom: '1rem', opacity: 0.9 }}>{destData.breadcrumb}</p>
          <h1 style={{
            fontSize: '4rem',
        ...
        <div style={{
          ...
          <span style={{ fontSize: '16px' }}>{dest.heading}</span>
        </div>
      </div>
      ...
      <div className="container" style={{ paddingTop: '3rem', paddingBottom: '3rem' }}>
          ...
          <div>
            ...
            <div style={{
              ...
              <div style={{
                ...
                {dest.coordinates ? (
                  <a
                    href={`https://www.google.com/maps?q=${dest.coordinates.lat},${dest.coordinates.lng}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    style={{ display: 'block', width: '100%', height: '100%' }}
                  >
                    <iframe
                      src={`https://www.google.com/maps/embed/v1/place?key=AIzaSyBFw0Qbyq9zTFTd-tUY6dZWTgaQzuU17R8&q=${dest.coordinates.lat},${dest.coordinates.lng}&center=${dest.coordinates.lat},${dest.coordinates.lng}&zoom=14`}
                      width="100%"
                      height="100%"
          ...
          <div style={{
            ...
            <div style={{ background: '#f8f9fa', borderRadius: '12px', padding: '1.5rem' }}>
              <div style={{
                ...
                <h3 style={{ fontSize: '1.5rem', fontFamily: 'Georgia, serif', margin: 0 }}>{destData.howToReach.byBus?.title || 'By Bus'}</h3>
              </div>
              <p style={{ fontSize: '14px', color: '#666', marginBottom: '1.25rem', lineHeight: '1.5' }}>
                {destData.howToReach.byBus?.description || ''}
              </p>
              <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                <a
                  href={dest.coordinates ? `https://www.google.com/maps/dir/?api=1&destination=${dest.coordinates.lat},${dest.coordinates.lng}&travelmode=driving` : '#'}
                  target="_blank"
                  rel="noopener noreferrer"
                  style={{
                    display: 'inline-flex',
            ...
            <div style={{ background: '#f8f9fa', borderRadius: '12px', padding: '1.5rem' }}>
              <div style={{
                ...
                <h3 style={{ fontSize: '1.5rem', fontFamily: 'Georgia, serif', margin: 0 }}>{destData.howToReach.byFlight?.title || 'By Flight'}</h3>
              </div>
              <p style={{ fontSize: '14px', color: '#666', marginBottom: '1.25rem', lineHeight: '1.5' }}>
                {destData.howToReach.byFlight?.description || ''}
              </p>
              <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                <a
                  href={dest.coordinates ? `https://www.google.com/maps/dir/?api=1&destination=${dest.coordinates.lat},${dest.coordinates.lng}&travelmode=driving` : '#'}
            ...
            <div style={{ background: '#f8f9fa', borderRadius: '12px', padding: '1.5rem' }}>
              <div style={{
                ...
                <span style={{ fontSize: '2rem' }}>🚂</span>
                <h3 style={{ fontSize: '1.5rem', fontFamily: 'Georgia, serif', margin: 0 }}>{destData.howToReach.byTrain?.title || 'By Train'}</h3>
              </div>
              <p style={{ fontSize: '14px', color: '#666', marginBottom: '1.25rem', lineHeight: '1.5' }}>
                {destData.howToReach.byTrain?.description || ''}
              </p>
              <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                <a
                  href={dest.coordinates ? `https://www.google.com/maps/dir/?api=1&destination=${dest.coordinates.lat},${dest.coordinates.lng}&travelmode=driving` : '#'}
        ...
        {dest.gallery && dest.gallery.length > 0 && (
          <div style={{ marginBottom: '4rem' }}>
            ...
            <p style={{
              ...
              Explore the beauty of {dest.heading} through our lens
            </p>
            <div style={{
              display: 'flex',
              gap: '1rem',
            ...
            onScroll={(e) => {
              ...
              dots.forEach((dot, i) => {
                dot.style.background = i === activeIndex ? '#FF9D00' : '#ddd';
              ...
              {dest.gallery.map((_, index) => (
                <div
                  ...
                  style={{
                    ...
                    height: '10px',
                    borderRadius: '50%',
                    background: index === 0 ? '#FF9D00' : '#ddd',
                    cursor: 'pointer',
                    transition: 'all 0.3s ease',
                    transform: index === 0 ? 'scale(1.3)' : 'scale(1)'
                  }}
                  onClick={() => {
                    const scroll = document.getElementById(`gallery-scroll-${dest.id}`);
                    if (scroll) {
        ...
        <div id="nearby" style={{ marginBottom: '4rem' }}>
          ...
          <div style={{
            ...
            {destData.nearbyDestinations.map((nearbyDest, index) => (
              <Link
                key={index}
                to={getNearbyDestinationPath(nearbyDest)}
                style={{
                  textDecoration: 'none',
                  color: 'inherit'
                }}
              >
                <div style={{
                  ...
                  <img
                    src={nearbyDest.image}
                    alt={nearbyDest.title}
                    style={{
                      width: '100%',
                      height: '100%',
                      objectFit: 'cover'
        ...
        {destData.nearbyDestinations.map((nearbyDest, nearbyIndex) => nearbyDest.gallery && nearbyDest.gallery.length > 0 ? (
          <div key={`nearby-gallery-${nearbyIndex}`} style={{ marginBottom: '4rem' }}>
            ...
            <p style={{
              ...
              Explore the beauty of {nearbyDest.title}
            </p>
            <div style={{
              display: 'flex',
              gap: '1rem',
            ...
            className="photo-gallery-scroll"
            id={`nearby-gallery-scroll-${nearbyIndex}`}
            onScroll={(e) => {
              const scroll = e.currentTarget;
              const maxScroll = scroll.scrollWidth - scroll.clientWidth;
              const scrollPercent = scroll.scrollLeft / maxScroll;
              ...
              {nearbyDest.gallery.map((imgUrl, index) => (
                ...
                >
                  ...
                  <div style={{
                    ...
                    {index + 1}/{nearbyDest.gallery.length}
                  </div>
                </div>
              ))}
            </div>
            {/* Dots */}
            <div
              id={`nearby-gallery-dots-${nearbyIndex}`}
              style={{
                display: 'flex',
                justifyContent: 'center',
                gap: '0.5rem',
            ...
            >
              {nearbyDest.gallery.map((_, index) => (
                <div
                  ...
                  onClick={() => {
                    const scroll = document.getElementById(`nearby-gallery-scroll-${nearbyIndex}`);
                    if (scroll) {
        ...
        <div id="hospitality" style={{ marginBottom: '4rem' }}>
          ...
          <p style={{
            ...
            Here's a wide range of accredited hospitality services available in {dest.heading}.
          </p>
...
export default NearbyDestinationDetail;